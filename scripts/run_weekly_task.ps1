...
$logFile = Join-Path $logDir "weekly_task_scheduler.log"
...
try {
    ...
    Add-Content -Path $logFile -Value "[$timestamp] Starting weekly pipeline"
    
    # Use Start-Process with separate stdout and stderr files
    $stdoutFile = "$logFile.stdout.tmp"
    $stderrFile = "$logFile.stderr.tmp"
    
    $process = Start-Process -FilePath "uv" -ArgumentList "run", "nse-analysis", "generate-weekly-report" `
    ...
    $timestamp = Get-Date -Format "yyyy-MM-dd HH:mm:ss"
    Add-Content -Path $logFile -Value "[$timestamp] Weekly pipeline finished"
    ...
    if (Get-Command git -ErrorAction SilentlyContinue) {
        $timestamp = Get-Date -Format "yyyy-MM-dd HH:mm:ss"
        Add-Content -Path $logFile -Value "[$timestamp] Committing weekly report"
        git add reports/weekly/*.md 2>&1 | Out-Null
        $null = git diff --cached --quiet reports/weekly/*.md 2>&1
        ...
        if ($hasChanges) {
            ...
            git commit -m "chore: add weekly NSE report" 2>&1 | Add-Content -Path $logFile
            git push 2>&1 | Add-Content -Path $logFile
            $timestamp = Get-Date -Format "yyyy-MM-dd HH:mm:ss"
            Add-Content -Path $logFile -Value "[$timestamp] Weekly report committed and pushed"
        } else {
