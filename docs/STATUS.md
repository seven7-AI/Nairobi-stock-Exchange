...
## #73 Corporate intelligence layer: entities, GLEIF, the resolver ladder, bitemporal helpers  ✅ 2026-09-19

Phase C2 of epic #70. `corporate_entities` and the first bitemporal table,
`corporate_relationships` (migration `20260919_0016`, autogenerated then reviewed).

- **`app/web/db/analytics/models/corporate_bitemporal.py`** — `BitemporalMixin`: valid
  time (`effective_from` / `effective_to` exclusive), transaction time (`recorded_at` /
  `superseded_at`), `superseded_by_id`, `supersession_reason`, `version_key`,
  `content_hash`, `source_id` / `source_kind`, `confidence`, `provenance`.
- **`app/web/services/corporate/bitemporal.py`** — `append_and_close`, the only writer:
  same key + content + source → `noop`; same content from a new source → the old
  version closed `corroborated`; different content → closed `restated`. `close_valid_time`
  ends a relationship with a **new** version (`closed`), never an UPDATE. `as_of`,
  `as_known_on`, `current` compose the two axes.
- **`entities/normalise.py`** — NFKD/ascii/lower, `&`→`and`, one legal suffix off;
  country words and the distinguishing tokens (`bank`, `insurance`, `health`, `capital`,
  …) kept; token-set similarity.
- **`entities/resolver.py`** — the ladder LEI → ISIN → registration → exchange id →
  canonical key → fenced fuzzy (same jurisdiction **and** group, ≥ 0.92, runner-up ≤ 0.80,
  no distinguishing token differs) → provisional (near miss, candidates recorded) or new.
  No jurisdiction → `ZZ` + queued, never Kenya.
- **`entities/gleif.py`** — `GleifClient` over `PoliteClient` with a disk cache; `by_lei`,
  `search(fulltext, country)`, `updated_since(day, country)` using
  `filter[registration.lastUpdateDate]=>date` (probed live: works, paginated).
- **`entities/service.py`** — `ensure_listed_entities` (one `listed` entity per company
  through the ladder, exchange id + ISIN in `identifiers`), `sync_gleif` (exact
  normalised-name match → LEI, GLEIF legal name, registration number at 1.0; fenced near
  match at 0.85 for review; miss → `field_sources.lei` at 0.0 with "checked <date>";
  retired records never match).
- CLI `corporate gleif sync [--ticker]`, `corporate entities show TICKER`.

Tests: `app/tests/unit/test_corporate_entities.py` — 33 tests: 8 normalisation cases; the
must-not-merge matrix (KCB Kenya/Tanzania, Equity Uganda/Rwanda, Jubilee/Jubilee Health,
Safaricom/Safaricom Ethiopia, NCBA Kenya/Uganda) and must-merge (`Limited` vs `Ltd`,
word order); mangled suffix → provisional with the candidate; fuzzy never crosses a group
or a jurisdiction; `ZZ` queued; LEI beats name, registration beats exact name; a
three-version restatement chain (inserted → corroborated → restated) queried by
`as_known_on` and `as_of`, old rows' `content_hash` unchanged; `close_valid_time`;
look-ahead (rows recorded later do not change an earlier belief); the GLEIF client on
recorded real responses (Safaricom, KCB Bank Kenya, KCB Tanzania, Equity Uganda,
NIC→NCBA retired, a 404), cache hit and expiry, delta paging; sync over the fixture
universe (no exact matches → 10 checked-not-absent; an exact EABL match writes LEI
`984500BEE8483EAED424`, `C.5/34`, and survives a universe rebuild); CLI; realdata.

Live (2026-09-19, store migrated 0015 → 0016): `corporate gleif sync` over the 81-company
universe — 81 listed entities created, 12 exact matches written at confidence 1.0 (ABSA
`2138006PDDMTP3WDX242`, Bamburi, BAT, DTB, EABL `984500BEE8483EAED424` reg `C.5/34`,
Family Bank, Kenya Airways, NBK, Standard Chartered Kenya, Safaricom
`984500A80B648A7B9717` reg `C.8/2002`, TotalEnergies Marketing Kenya via its former name
in GLEIF's other names, Unilever Tea Kenya), 69 recorded as checked with no record — KCB
Group, Equity Group, NCBA Group, Co-op Bank and Britam have no LEI at all, so GLEIF stays
enrichment. 81 requests at 3 s spacing, cached for 7 days.
